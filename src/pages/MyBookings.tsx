...
const MyBookings = () => {
  ...
  // Function to check if a booking is completed
  const isBookingCompleted = (booking: Booking) => {
    ...
    try {
      ...
      // Add timezone offset to handle local time correctly
  ...
  const updateCompletedBookings = async () => {
    ...
    try {
      const completedBookings = bookings.filter(
        booking => booking.status === 'confirmed' && isBookingCompleted(booking)
      );

      console.log("Found completed bookings:", completedBookings.map(b => ({
        id: b.id,
        date: b.booking_date,
        endTime: b.end_time
      })));
      ...
      if (completedBookings.length > 0) {
        const { error } = await supabase
          ...
          .in('id', completedBookings.map(b => b.id));
        ...
        setBookings(prevBookings => 
          prevBookings.map(booking => 
            completedBookings.some(b => b.id === booking.id)
              ? { ...booking, status: 'completed' }
              : booking
          )
        );

        console.log("Successfully updated bookings to completed");
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    const fetchBookings = async () => {
      try {
        ...
        // Immediately check for completed bookings after fetching
        const bookingsData = data as Booking[] || [];
        const completedOnes = bookingsData.filter(
          booking => booking.status === 'confirmed' && isBookingCompleted(booking)
        );
        
        if (completedOnes.length > 0) {
          console.log("Found bookings to mark as completed:", completedOnes.length);
